        ...
        throws JSONException
    {
        ...
        if (flag)
        {
            cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run()
                {
                    PluginResult pluginresult2 = execute(action, args, callbackId);
                    PluginResult pluginresult1 = pluginresult2;
_L2:
                    sendPluginResult(pluginresult1, callbackId);
                    return;
                    Throwable throwable;
                    throwable;
                    pluginresult1 = new PluginResult(PluginResult.Status.ERROR, throwable.getMessage());
...
_L1:
        ...
        PluginResult pluginresult = execute(action, args, callbackId);
